...
where
a.ETL_SYSTEM  in ('BOS') 
and a.ETL_JOB in (select Dependency_Job from odsetl.ETL_Job_Dependency where ETL_Job IN ('T_EVT_EVENT_TGPS'))
UNION ALL
...
where
a.ETL_SYSTEM  in ('BOS') 
and a.ETL_JOB in (select ETL_Job from odsetl.ETL_Job_Stream where Stream_Job IN ('T_EVT_EVENT_TGPS'));
